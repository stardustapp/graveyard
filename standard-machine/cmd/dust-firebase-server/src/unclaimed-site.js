...
exports.UnclaimedSite = class UnclaimedSite {
  constructor(context) {
    if (!context) throw new Error(
      `UnclaimedSite requires a context`);
    ...
    // dynamic pages
    this.koa.use(route.get('/', ctx => new UnclaimedSiteLogin(this).get(ctx)));
    this.koa.use(route.get('/claim', ctx => new UnclaimedSiteRegister(this).get(ctx)));
    this.koa.use(route.post('/login', ctx => new UnclaimedSiteLogin(this).post(ctx)));

    // assets
    this.koa.use(route.get('/style.css', ctx => new UnclaimedSiteStyle().get(ctx)));
  }
...
class UnclaimedSiteCreatePassword {
  constructor(site) {
    this.site = site;
  }
...
// class UnclaimedSiteManageDomain {
...
//
//   async post(input) {
//     const request = await new UnclaimedSiteRequest(this.site, input).loadState();
//     if (!request.session) {
//       return ctx.redirect(ctx.request.origin+'/~/login');
//     }
...
class UnclaimedSiteAppSessionApi {
  constructor(site) {
    this.site = site;
  }
...
class UnclaimedSiteServiceWorker {
  async get(ctx) {
