class NetworkImportMount {
  constructor(opts) {
    ...
    if (!opts.url.startsWith('ws')) {
      throw new Error(`network-import does not support http yet`);
    }
  ...
  async init() {
    this.transport = new SkylinkWsTransport(this.url, {}, true);
    await this.transport.connPromise;
  }

  async getEntry(path) {
    return new ImportedEntry(this, this.prefix+path);
  }
}
